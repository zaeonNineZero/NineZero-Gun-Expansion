 ...
 */
public class BullpupRifleModel implements IOverrideModel
{
	...
	// We start by declaring our render function that will handle rendering all model components.
    public void render(float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack, ItemStack parent, @Nullable LivingEntity entity, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
		// Render the item's BakedModel, which will serve as the core of our custom model.
        BakedModel bakedModel = SpecialModels.BULLPUP_RIFLE_BASE.getModel();
		...
		{
            RenderUtil.renderModel(SpecialModels.BULLPUP_RIFLE_SIGHTS.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
		}
        ...
        if(isPlayer && correctContext)
        {
            ...
            boltTranslations = boltTranslations.add(0, 0, cooldown_d * 2.5);
        }
        ...
        RenderUtil.renderModel(SpecialModels.BULLPUP_RIFLE_BOLT_HANDLE.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
		...
		// Render the transformed model.
        SpecialModels magModel = SpecialModels.BULLPUP_RIFLE_MAGAZINE;
        try {
            ...
            if(!magStack.isEmpty())
            {
	            if (magStack.getItem().builtInRegistryHolder().key().location().getPath().equals("light_magazine"))
		    		magModel = SpecialModels.BULLPUP_RIFLE_LIGHT_MAG;
	            else
	            if (magStack.getItem().builtInRegistryHolder().key().location().getPath().equals("extended_magazine"))
			    	magModel = SpecialModels.BULLPUP_RIFLE_EXTENDED_MAG;
            }
		}
